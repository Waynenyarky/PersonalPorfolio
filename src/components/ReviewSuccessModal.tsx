				...
				<div className="p-6 sm:p-8 lg:p-10 text-center">
					...
					<div className="mb-4 sm:mb-6 flex justify-center">
						<div className="relative">
							<div className="w-16 h-16 sm:w-20 sm:h-20 bg-linear-to-br from-green-500 via-emerald-500 to-teal-600 rounded-full flex items-center justify-center shadow-lg shadow-green-500/30 animate-scale-in">
								<CheckCircle className="text-white w-[clamp(28px,8vw,40px)] h-[clamp(28px,8vw,40px)]" size={32} strokeWidth={2.5} />
							</div>
							{/* Pulse animation rings */}
							<div className="absolute inset-0 rounded-full bg-green-500/20 animate-ping" />
							<div className="absolute inset-0 rounded-full bg-green-500/10 animate-ping [animation-delay:0.5s]" />
						</div>
					</div>
					...
					{/* Rating Display */}
					{rating && (
						<div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-linear-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-orange-950/30 dark:via-amber-950/20 dark:to-yellow-950/30 rounded-xl border border-orange-200/50 dark:border-orange-900/30">
							...
							<div className="flex items-center justify-center gap-0.5 sm:gap-1">
								{[...Array(5)].map((_, i) => (
									<Star
										key={i}
										className={`w-[clamp(18px,5vw,24px)] h-[clamp(18px,5vw,24px)] ${i < rating ? 'text-orange-500 fill-orange-500' : 'text-gray-300 dark:text-gray-600'}`}
										size={20}
									/>
								))}
